...
class Bottleneck(nn.Module):
    expansion = 4
    def __init__(self, in_dims, out_dims, stride=1, downsampple=None):
        super(Bottleneck, self).__init__()
        self.bottleneck = nn.Sequential(
            nn.Conv2d(in_dims, out_dims, 1, bias=False),
            nn.BatchNorm2d(out_dims),
            nn.ReLU(inplace=True),
            nn.Conv2d(out_dims, out_dims, 3, stride, 1, bias=False),
            nn.BatchNorm2d(out_dims),
            nn.ReLU(inplace=True),
            nn.Conv2d(out_dims, self.expansion * out_dims, 1, bias=False)
        )
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsampple
